...
public:
  ...
  public:
    // WARNING: Mutable
    arraylayout layout; // number of dimensions ndim is layout.dimlen.size()
    ...
    arrayposition(const arraylayout &layout,snde_index intpos,bool fortran_indexing=false) :
      layout(layout),
      pos(std::max(layout.dimlen.size(),1),0),
    fortran_indexing(fortran_indexing)
    {
    ...
    arrayposition(const arraylayout &layout,std::vector<snde_index> pos,bool fortran_indexing=false) :
      ...
      fortran_indexing(fortran_indexing)
    {
      assert((layout.ndim.size() == 0  &&  pos.size() == 1) || (layout.ndim.size() == pos.size()));
      assert(pos.size() >= 1);
    }
