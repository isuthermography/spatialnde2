...
static inline  std::tuple<snde_index,std::set<snde_index>> enclosed_or_intersecting_polygons_3d(std::set<snde_index> & polys,const snde_triangle *part_triangles,const snde_edge *part_edges,const snde_coord3 *part_vertices,const snde_mat23 *part_inplanemats,const snde_coord3 *part_trinormals,snde_coord3 box_v0,snde_coord3 box_v1)
  ...
  //size_t poolidx;
    ...
    for (polys_it=polys.begin();polys_it != polys.end();polys_it=polys_next_it) {
      ...
      get_we_triverts(part_triangles,idx,part_edges,part_vertices,tri_vertices);
  ...
  // returns index of the next available box to build
  {
    ...
    if (num_fully_enclosed > 10 && depth <= 22) {
      ...
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx,miny,minz,minx+distx/2.0+eps,miny+disty/2.0+eps,minz+distz/2.0+eps);
      boxlist[cnt][1]=newcnt;
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx+distx/2.0-eps,miny,minz,maxx,miny+disty/2.0+eps,minz+distz/2.0+eps);
      boxlist[cnt][2]=newcnt;
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx,miny+disty/2.0-eps,minz,minx+distx/2.0+eps,maxy,minz+distz/2.0+eps);
      boxlist[cnt][3]=newcnt;
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx+distx/2.0-eps,miny+disty/2.0-eps,minz,maxx,maxy,minz+distz/2.0+eps);
      boxlist[cnt][4]=newcnt;
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx,miny,minz+distz/2.0-eps,minx+distx/2.0+eps,miny+disty/2.0+eps,maxz);
      boxlist[cnt][5]=newcnt;
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx+distx/2.0-eps,miny,minz+distz/2.0-eps,maxx,miny+disty/2.0+eps,maxz);
      boxlist[cnt][6]=newcnt;
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx,miny+disty/2.0-eps,minz+distz/2.0-eps,minx+distx/2.0+eps,maxy,maxz);
      boxlist[cnt][7]=newcnt;
      newcnt = _buildbox(geom,partstruct,boxlist,boxcoordlist,ourpolys,boxpolylist,newcnt,depth+1,minx+distx/2.0-eps,miny+disty/2.0-eps,minz+distz/2.0-eps,maxx,maxy,maxz);
  ...
  std::tuple<
    ...
    std::vector<snde_index>> build_boxes(std::shared_ptr<geometry> geom,const snde_part &partstruct)
  ...
  // returns <boxlist,boxcoordlist,boxpolylist>
  {
    ...
    _buildbox(geom,partstruct,boxlist,boxcoordlist,polys,boxpolylist,0,0,minx-eps,miny-eps,minz-eps,maxx+eps,maxy+eps,maxz+eps);
...
std::shared_ptr<trm_dependency> boxes_calculation(std::shared_ptr<geometry> geom,std::shared_ptr<trm> revman,std::shared_ptr<snde::component> comp,cl_context context,cl_device_id device,cl_command_queue queue)
{
  ...
  return revman->add_dependency_during_update(
						...
						if (actions & STDA_EXECUTE) {
						  ...
						  comp->obtain_geom_lock(lockprocess,
									 ...
									 SNDE_COMPONENT_GEOM_PARTS|SNDE_COMPONENT_GEOM_BOXES|SNDE_COMPONENT_GEOM_BOXPOLYS);
						...
						if (actions & STDA_EXECUTE) {
						  ...
						  std::tie(boxlist,boxcoordlist,boxpolylist)=build_boxes(geom,partstruct);
