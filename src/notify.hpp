...
Waveform database notification scenarios: 
...
1. channel_notify and derivatives. This uses Method B with notifications in channel_state::_notify_about_this_channel_metadataonly
   ...
   based on the internal completion flags in waveforms and channel_state::issue_nonmath_notifications()
2. Waveform math notifications within a particular waveform_set_state: Method A. These are used to trigger dependent functions and rely on
...
3. Waveform math and channel notifications from a different waveform_set_state: Method A:
...
namespace snde {
  // forward references
  class waveform_state_set; //wfmstore.hpp
  class wfmdatabase; //wfmstore.hpp
  class globalrevision; //wfmstore.hpp
