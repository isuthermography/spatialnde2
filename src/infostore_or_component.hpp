...
namespace snde {
  ...
  public:
    ...
    virtual void obtain_geom_lock(std::shared_ptr<lockingprocess> process, std::shared_ptr<iterablewfmrefs> wfmdb_wfmlist=nullptr,std::string wfmdb_context="/",snde_infostore_lock_mask_t readmask=SNDE_COMPONENT_GEOM_ALL,snde_infostore_lock_mask_t writemask=0,snde_infostore_lock_mask_t resizemask=0)=0;
    virtual void obtain_uv_lock(std::shared_ptr<lockingprocess> process, std::shared_ptr<iterablewfmrefs> wfmdb_wfmlist=nullptr,std::string wfmdb_context="/",snde_infostore_lock_mask_t readmask=SNDE_COMPONENT_GEOM_ALL,snde_infostore_lock_mask_t writemask=0,snde_infostore_lock_mask_t resizemask=0)=0;
  ...
  obtain_graph_lock_instances_multiple(std::shared_ptr<lockingprocess> process,
				       ...
				       std::shared_ptr<mutablewfmdb> wfmdb=nullptr,
    ...
    static inline std::shared_ptr<iterablewfmrefs> obtain_graph_lock(std::shared_ptr<lockingprocess> process,
				  std::string root,
				  std::vector<std::string> extra_channels,
				  std::set<std::shared_ptr<lockable_infostore_or_component>,std::owner_less<std::shared_ptr<lockable_infostore_or_component>>> extra_components,  // NOTE: Does NOT traverse the graph of extra_components (unless reached by other means)
				  std::shared_ptr<mutablewfmdb> wfmdb=nullptr,
				  ...
				  snde_infostore_lock_mask_t writemask=0)
    {
      ...
      std::tie(wfmdb_wfmlist,instancearray)=obtain_graph_lock_instances_multiple(process,
										 roots,
										 ...
										 wfmdb=wfmdb,wfmdb_context=wfmdb_context,readmask=readmask,writemask=writemask);

      return wfmdb_wfmlist;
    }
  ...
  static inline std::shared_ptr<iterablewfmrefs> obtain_graph_lock(std::shared_ptr<lockingprocess> process,
    ...
    static inline std::shared_ptr<iterablewfmrefs> obtain_graph_lock_uv_imagedata(std::shared_ptr<lockingprocess> process,
									   std::string root,
									   std::vector<std::string> extra_channels,
									   std::set<std::shared_ptr<lockable_infostore_or_component>,std::owner_less<std::shared_ptr<lockable_infostore_or_component>>> extra_components,  // NOTE: Does NOT traverse the graph of extra_components (unless reached by other means)
									   std::shared_ptr<immutable_metadata> metadata,
										  std::function<std::tuple<std::shared_ptr<parameterization>,std::map<snde_index,std::tuple<std::shared_ptr<mutabledatastore>,std::shared_ptr<image_data>>>>(std::shared_ptr<iterablewfmrefs> wfmdb_wfmlist,std::shared_ptr<part> partdata,std::vector<std::string> uv_imagedata_names)> get_uv_imagedata,
									   std::shared_ptr<mutablewfmdb> wfmdb=nullptr,std::string wfmdb_context="/",snde_infostore_lock_mask_t readmask=0,snde_infostore_lock_mask_t writemask=0)
    {
  ...
  static inline std::tuple<std::shared_ptr<iterablewfmrefs>,std::vector<std::tuple<snde_partinstance,std::shared_ptr<part>,std::shared_ptr<parameterization>,std::map<snde_index,std::tuple<std::shared_ptr<mutabledatastore>,std::shared_ptr<image_data>>>>>>
				...
				std::shared_ptr<mutablewfmdb> wfmdb=nullptr,std::string wfmdb_context="/",snde_infostore_lock_mask_t readmask=0,snde_infostore_lock_mask_t writemask=0)
    {
      ...
      return obtain_graph_lock_instances_multiple(process,
						  ...
						  metadata,
						  get_uv_imagedata,
						  wfmdb=wfmdb,wfmdb_context=wfmdb_context,
						  readmask=readmask,writemask=writemask);
      // returns (wfmdb_wfmlist,instancearray)
    }
