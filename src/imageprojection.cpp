...
#include "raytrace_h.h"
#include "imageprojection_calc_c.h"
...
namespace snde {
  ...
  std::shared_ptr<trm_dependency> imageprojection(std::shared_ptr<geometry> geom,std::shared_ptr<trm> revman,std::shared_ptr<mutablegeomstore> comp,std::shared_ptr<mutabledatastore> to_project,cl_context context,cl_device_id device,cl_command_queue queue)
{
  ...
  return revman->add_dependency_during_update(
						...
						comp->obtain_lock(lockprocess); // components precede parameterizations in locking order
