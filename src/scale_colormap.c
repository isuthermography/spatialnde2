			     ...
			     float DivPerUnits)
{
  ...
  snde_rgba out;
  float scaledval=(inarray[input_offset + stride_x*xidx + stride_y*yidx]-offset)*DivPerUnits;

  outimage[xidx + get_global_size(0)*yidx] = snde_colormap(ColorMap,scaledval,alpha);
}
