...
namespace snde {
...
#ifdef SNDE_OPENCL
  static opencl_program areacalc_trianglearea_opencl("snde_areacalc_triangleareas", { snde_types_h, geometry_types_h, vecops_h, geometry_ops_h, area_calc_c });
#endif // SNDE_OPENCL
	...
	return std::make_shared<lock_alloc_function_override_type>([ this,result_ref,recording ]() {
	  ...
	  return std::make_shared<exec_function_override_type>([ this,locktokens, result_ref, recording,numtris  ]() {
	    ...
	    if (opencl_resource) {
	      ...
	      cl::Kernel triarea_kern = areacalc_trianglearea_opencl.get_kernel(opencl_resource->context,opencl_resource->devices.at(0));
  ...
  void instantiate_trianglearea(std::shared_ptr<recdatabase> recdb,std::shared_ptr<loaded_part_geometry_recording> loaded_geom)
  {
	...
	std::make_shared<math_parameter_recording>("meshedpart"),
      },
      {
