...
namespace snde {
  ...
  {
  public:
    colormap_recording(std::shared_ptr<recording_set_state> wss,std::shared_ptr<instantiated_math_function> inst) :
      recmath_cppfuncexec<std::shared_ptr<ndtyped_recording_ref<T>>,int,snde_float64,snde_float64,std::vector<snde_index>,unsigned,unsigned>(wss,inst)
    {
    ...
    std::shared_ptr<metadata_function_override_type> define_recs(std::shared_ptr<ndtyped_recording_ref<T>> recording, int colormap_type,snde_float64 offset, snde_float64 unitsperintensity,std::vector<snde_index> base_position,unsigned u_dim,unsigned v_dim) 
    {
      ...
      std::shared_ptr<ndtyped_recording_ref<snde_rgba>> result_rec = ndtyped_recording_ref<snde_rgba>::create_recording_math(this->get_result_channel_path(0),this->wss);
  ...
  std::shared_ptr<math_function> define_colormap_recording_function()
  {
    ...
    // or CPU version
    return std::make_shared<cpp_math_function>([] (std::shared_ptr<recording_set_state> wss,std::shared_ptr<instantiated_math_function> inst) {
      // ***!!! Should extend beyond just floating types
      // ***!!! Should modify to be able to pass OpenCL context, etc. in 
      return make_cppfuncexec_floatingtypes<colormap_recording>(wss,inst);
    },
      true,
      false,
      false);
