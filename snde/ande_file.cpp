...
namespace snde {
  ...
  std::shared_ptr<recording_base> andefile_readarray::define_rec(std::shared_ptr<active_transaction> trans,std::string ownername,void *owner_id)
  {
    ...
    if (!loadchan) {
      loadchan = trans->recdb->define_channel(trans,recpath,ownername,owner_id); // Note: no way (so far) to set hidden flag or storage manager
    }
    
    std::shared_ptr<multi_ndarray_recording> retval = create_recording<multi_ndarray_recording>(trans,loadchan,owner_id,(size_t)numarrays);
  ...
  std::shared_ptr<recording_base> andefile_readgroup::define_rec(std::shared_ptr<active_transaction> trans,std::string ownername,void *owner_id)
  {
    std::shared_ptr<channel> loadchan;
    ...
    std::shared_ptr<recording_group> retval = create_recording<recording_group>(trans,loadchan,owner_id); //,nullptr);
    retval->metadata=metadata;
  ...
  std::shared_ptr<ande_loadrecording_map> andefile_loadfile(std::shared_ptr<active_transaction> trans,std::string ownername,void *owner_id,std::string filename,std::string recpath /* ="/" */ ) // add filter function parameter or specific recording to request to limit what is loaded? 
  {
    ...
    for (auto && recname_loaderptr_recordingptr: *filemap) {
      ...
      loaderptr_recordingptr.second = loaderptr_recordingptr.first->define_rec(trans,ownername,owner_id);
    }
