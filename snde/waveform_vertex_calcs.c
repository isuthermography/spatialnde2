	...
	snde_float32 A)
{
	...
	snde_index stidx = (pos - 1) * pxstep;
	waveform_intype minval = -log(0.0);
	waveform_intype maxval = -minval;
	for (snde_index i = stidx; i < stidx + pxstep; ++i) {
		if (inputs[i] > maxval) {
			maxval = inputs[i];
		}
		if (inputs[i] < minval) {
	...
	priorx = step * (waveform_intype)(pos - 1) + inival + 0.5 * step;
	curx = priorx;
	priory = minval;	
	cury = maxval;
